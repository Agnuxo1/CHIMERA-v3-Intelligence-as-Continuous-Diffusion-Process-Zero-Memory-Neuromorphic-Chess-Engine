...
class BrainStateManager:
    ...
    def create_initial_brain(self):
        ...
        # 5. Tactical patterns (region 32-64, 0-32) - MASTER-LEVEL KNOWLEDGE
        # Common tactical motifs: forks, pins, skewers, discovered attacks
        # Encode patterns that lead to winning positions
        
        # Fork patterns (knight/pawn attacking two pieces)
        fork_patterns = [
            # Knight fork squares (central positions)
            (3, 3), (4, 3), (3, 4), (4, 4),  # Center forks
            (2, 2), (5, 2), (2, 5), (5, 5),  # Extended center
        ]
        for x, y in fork_patterns:
            brain[y, 32 + x, 0] = 0.75  # High tactical value
            brain[y, 32 + x, 2] = 0.6   # Positive evaluation
            brain[y, 32 + x, 3] = 0.85  # High confidence
        
        # Pin patterns (pieces aligned with king)
        # These are encoded as directional patterns
        for y in range(8):
            for x in range(8):
                # Pins are valuable when pieces are aligned
                if (x == 3 or x == 4) and (y == 3 or y == 4):
                    brain[y, 40 + x, 0] = 0.7
                    brain[y, 40 + x, 2] = 0.55
                    brain[y, 40 + x, 3] = 0.8
        
        # 6. Opening principles (region 48-64, 0-16) - MASTER OPENINGS
        # Encode common opening moves and principles
        
        # Popular opening moves (e4, d4, Nf3, etc.)
        opening_moves = [
            (4, 4),  # e4 (most popular)
            (3, 4),  # d4 (second most popular)
            (5, 5),  # Nf3
            (2, 5),  # Nc3
            (6, 6),  # Bf4
            (1, 5),  # b3
        ...
        for x, y in opening_moves:
            brain[y, 48 + x, 0] = 0.8   # Very high value
            brain[y, 48 + x, 2] = 0.65  # Strong positive
            brain[y, 48 + x, 3] = 0.9   # Very high confidence
        
        # 7. Endgame knowledge (region 64-96, 0-32) - MASTER ENDGAMES
        # King activity, pawn promotion, opposition
        
        # King activity in endgame (center is key)
        for x in range(3, 6):
            for y in range(3, 6):
                brain[y, 64 + x, 0] = 0.7
                brain[y, 64 + x, 2] = 0.6
        ...
        # Pawn promotion squares (7th/2nd rank)
        for x in range(8):
            brain[1, 72 + x, 0] = 0.85  # White 7th rank
            brain[6, 72 + x, 0] = 0.85  # Black 2nd rank
            brain[1, 72 + x, 2] = 0.7
            brain[6, 72 + x, 2] = 0.7
            brain[1, 72 + x, 3] = 0.9
            brain[6, 72 + x, 3] = 0.9
        
        # 8. Piece coordination patterns (region 80-96, 0-32)
        # Rooks on open files, bishops on long diagonals
        
        # Open file patterns (files d, e are most valuable)
        for y in range(8):
            brain[y, 80 + 3, 0] = 0.75  # d-file
            brain[y, 80 + 4, 0] = 0.75  # e-file
            brain[y, 80 + 3, 2] = 0.6
            brain[y, 80 + 4, 2] = 0.6
            brain[y, 80 + 3, 3] = 0.85
            brain[y, 80 + 4, 3] = 0.85
        
        # 9. Advanced positional concepts (region 96-128, 0-32)
        # Weak squares, outposts, piece activity
        
        # Weak square patterns (squares that can't be defended by pawns)
        weak_squares = [
            (3, 3), (4, 3), (3, 4), (4, 4),  # Center weak squares
            (2, 2), (5, 2), (2, 5), (5, 5),  # Extended weak squares
        ]
        for x, y in weak_squares:
            brain[y, 96 + x, 0] = 0.7
            brain[y, 96 + x, 2] = 0.55
            brain[y, 96 + x, 3] = 0.8
        
        # 10. Material imbalance knowledge (region 112-128, 0-32)
        # When to trade, piece values in different phases
        
        # Piece values encoded (for trading decisions)
        # R channel: opening value, G channel: endgame value
        piece_values = {
            1: (1.0, 1.0),    # Pawn: same in opening/endgame
            2: (3.0, 3.0),    # Knight: same
            3: (3.2, 3.2),    # Bishop: slightly better
            4: (5.0, 5.0),    # Rook: same
            5: (9.0, 9.0),    # Queen: same
        }
        for piece_type, (opening_val, endgame_val) in piece_values.items():
            normalized = piece_type / 12.0
            brain[0, 112 + piece_type, 0] = opening_val / 20.0
            brain[0, 112 + piece_type, 1] = endgame_val / 20.0
            brain[0, 112 + piece_type, 3] = 0.95
        
        # 11. Attack patterns (region 128-160, 0-32)
        # King attacks, piece attacks on weak squares
        
        # King attack patterns (when king is exposed)
        attack_squares = [
            (3, 2), (4, 2), (5, 2),  # Attacking f7/f2
            (2, 3), (5, 3),          # Attacking g7/g2
        ]
        for x, y in attack_squares:
            brain[y, 128 + x, 0] = 0.8
            brain[y, 128 + x, 2] = 0.65
            brain[y, 128 + x, 3] = 0.9
...
class CHIMERAChessEngineV2:
    ...
    def __init__(self, ctx: moderngl.Context):
        ...
        # Learning: Store game positions for learning
        self.game_positions = []  # List of (board_array, evaluation) tuples from current game
    ...
    def _compile_shaders(self):
        ...
        self.brain_evolution_shader = self.ctx.compute_shader('''
            ...
            void main() {
                ...
                // MASTER-LEVEL EVALUATION with comprehensive knowledge
                float eval_base = 0.6 * center.b + 0.4 * neighbors.b;
                
                // Multi-layer knowledge influence
                float knowledge_bias = knowledge.r - knowledge.g;  // Win - loss patterns
                float knowledge_eval = knowledge.b * knowledge.a;   // Learned evaluations
                float knowledge_confidence = knowledge.a;            // Pattern confidence
                
                // Positional factors from knowledge base
                float position_factor = position.r * 0.15;
                float spatial_factor = spatial.r * 0.1;  // Pattern recognition
                
                // Advanced evaluation combining all knowledge sources
                // Higher weight for high-confidence patterns
                float knowledge_weight = 0.4 * knowledge_confidence;
                float combined_eval = eval_base + 
                                     knowledge_bias * knowledge_weight +
                                     knowledge_eval * 0.3 +
                                     position_factor +
                                     spatial_factor * 0.2;
                
                evolved.b = tanh(combined_eval);
        ...
        self.batch_evaluation_shader = self.ctx.compute_shader('''
            ...
            void main() {
                ...
                // MASTER-LEVEL POSITIONAL EVALUATION
                float center_bonus = 0.0;
                if ((pos.x >= 2 && pos.x <= 5) && (pos.y >= 2 && pos.y <= 5)) {
                    center_bonus = 0.4;  // Increased center value
                }
                
                // Advanced positional factors from knowledge base
                float development_bonus = knowledge.g * 0.3;  // Development patterns
                float tactical_bonus = 0.0;
                
                // Check for tactical patterns (forks, pins, etc.)
                if (pos.x >= 32 && pos.x < 64) {
                    tactical_bonus = knowledge.r * 0.4;  // Tactical pattern value
                }
                
                // Opening/endgame knowledge
                float phase_bonus = 0.0;
                if (pos.x >= 48 && pos.x < 64) {
                    phase_bonus = knowledge.r * 0.25;  // Opening principles
                }
                if (pos.x >= 64 && pos.x < 96) {
                    phase_bonus = knowledge.r * 0.3;   // Endgame knowledge
                }
                ...
                if (thread_id == 0) {
                    ...
                    // MASTER-LEVEL EVALUATION: Adaptive weights based on game phase
                    // Opening: more positional, Endgame: more material
                    // We estimate phase from material count (fewer pieces = endgame)
                    float material_count = abs(total_material);
                    float game_phase = clamp(material_count / 30.0, 0.0, 1.0);  // 0=endgame, 1=opening
                    
                    // Adaptive weights: more positional in opening, more material in endgame
                    float material_weight = 0.5 + 0.2 * (1.0 - game_phase);  // 0.5-0.7
                    float position_weight = 0.5 + 0.2 * game_phase;           // 0.5-0.7
                    
                    float eval = total_material * material_weight + 
                                total_position * position_weight;
                    
                    // Normalize to reasonable range [-20, +20] then to [-1, 1]
                    eval = tanh(eval / 15.0);
                    
                    imageStore(evaluation_out, ivec2(position_index, 0), 
                              vec4(eval, total_material, total_position, game_phase));
                }
            }
        ''')
        
        # ====================================================================
        # SHADER 5: LEARNING (updates BASE_BRAIN.png with winning patterns)
        # ====================================================================
        
        self.learning_shader = self.ctx.compute_shader('''
            #version 430
            ...
            layout(rgba32f, binding = 1) uniform image2D knowledge_base;
            layout(rgba32f, binding = 2) uniform image2D knowledge_updated;
            ...
            void main() {
                ivec2 pos = ivec2(gl_GlobalInvocationID.xy);
                ivec2 size = imageSize(knowledge_base);
                
                if (pos.x >= size.x || pos.y >= size.y) return;
                
                vec4 current_knowledge = imageLoad(knowledge_base, pos);
                vec4 pattern = imageLoad(position_pattern, pos);
                
                // Only update board region and knowledge regions (8×8 and knowledge regions)
                vec4 updated = current_knowledge;
                
                // Evolutionary learning: update knowledge based on game outcome
                // R channel: Winning patterns (increase if outcome > 0)
                // G channel: Losing patterns (increase if outcome < 0)
                // B channel: Learned evaluations (weighted average)
                // A channel: Confidence (increases with frequency)
                
                if (pos.x < 8 && pos.y < 8) {
                    // Board region: update winning/losing patterns
                    if (outcome > 0.0) {
                        // Won: strengthen winning patterns
                        updated.r = current_knowledge.r * (1.0 - learning_rate) + 
                                   pattern.r * learning_rate * outcome;
                        updated.g = current_knowledge.g * (1.0 - learning_rate * 0.5);  // Weaken losing
                    } else if (outcome < 0.0) {
                        // Lost: strengthen losing patterns (to avoid)
                        updated.g = current_knowledge.g * (1.0 - learning_rate) + 
                                   pattern.r * learning_rate * abs(outcome);
                        updated.r = current_knowledge.r * (1.0 - learning_rate * 0.5);  // Weaken winning
                    }
                    
                    // Update learned evaluations (weighted average)
                    float eval_weight = pattern.b;  // Evaluation from pattern
                    updated.b = current_knowledge.b * (1.0 - learning_rate) + 
                               eval_weight * learning_rate;
                    
                    // Update confidence (increases with frequency)
                    updated.a = min(1.0, current_knowledge.a + learning_rate * 0.1);
                } else {
                    // Knowledge regions: update tactical/positional patterns
                    // Similar update but with lower learning rate for stability
                    float region_learning_rate = learning_rate * 0.5;
                    
                    if (outcome > 0.0) {
                        updated.r = current_knowledge.r * (1.0 - region_learning_rate) + 
                                   pattern.r * region_learning_rate * outcome;
                    } else if (outcome < 0.0) {
                        updated.g = current_knowledge.g * (1.0 - region_learning_rate) + 
                                   pattern.r * region_learning_rate * abs(outcome);
                    }
                    
                    updated.b = current_knowledge.b * (1.0 - region_learning_rate) + 
                               pattern.b * region_learning_rate;
                    updated.a = min(1.0, current_knowledge.a + region_learning_rate * 0.05);
                }
                
                imageStore(knowledge_updated, pos, updated);
            }
        ''')
        
        # ====================================================================
        # SHADER 6: SPATIAL FEATURES (3×3 pattern detection)
        # ====================================================================
    ...
    def find_best_move(self, board_array: np.ndarray, white_to_move: bool, depth: int = 3, game_move_count: int = 0) -> Optional[Move]:
        ...
        # Store current position for learning (CHIMERA's positions only)
        if not white_to_move:  # CHIMERA is black
            current_eval = self.evaluate_position_gpu(board_array)
            self.game_positions.append((board_array.copy(), current_eval))
        
        # MASTER-LEVEL MINIMAX SEARCH (GPU-accelerated)
        def minimax(board_arr: np.ndarray, is_white: bool, current_depth: int, 
                   alpha: float, beta: float, maximizing: bool) -> float:
            """Minimax with alpha-beta pruning - all evaluation on GPU"""
            if current_depth == 0:
                # Evaluate position on GPU
                eval_score = self.evaluate_position_gpu(board_arr)
                return eval_score if maximizing else -eval_score
            
            # Generate moves on GPU
            moves = self.generate_legal_moves_gpu(board_arr, is_white, game_move_count)
        ...
        for i, move in enumerate(legal_moves[:30]):  # Limit for performance
            # Create new board
            new_board = board_array.copy()
            new_board[move.to_y, move.to_x] = move.piece
            new_board[move.from_y, move.from_x] = Piece.EMPTY
            
            # Evaluate with minimax (GPU-accelerated evaluation)
            score = minimax(new_board, not white_to_move, depth - 1, 
                          float('-inf'), float('inf'), not white_to_move)
            ...
            # Print progress
            if (i + 1) % 5 == 0:
                print(f"  Evaluated {i+1}/{min(30, len(legal_moves))} moves...")
    ...
    def learn_from_game(self, outcome: float):
        """
        Update brain's knowledge based on game outcome - 100% GPU
        outcome: +1.0 (win), 0.0 (draw), -1.0 (loss)
        
        This updates BASE_BRAIN.png with winning/losing patterns
        """
        ...
        knowledge_data = np.frombuffer(self.knowledge_texture.read(), dtype=np.float32)
        knowledge_data = knowledge_data.reshape((256, 256, 4))
        
        # Create temporary texture for updated knowledge
        updated_knowledge_texture = self.ctx.texture(TEXTURE_SIZE, 4, dtype='f4')
        updated_knowledge_texture.write(knowledge_data.tobytes())
        
        # Learn from each position in the game
        for board_array, evaluation in self.game_positions:
            # Encode position to texture
            self.encode_board_to_texture(board_array)
            
            # Bind textures for learning shader
            self.brain_texture.bind_to_image(0, read=True, write=False)
            self.knowledge_texture.bind_to_image(1, read=True, write=False)
            updated_knowledge_texture.bind_to_image(2, read=False, write=True)
            
            # Set learning parameters
            self.learning_shader['learning_rate'].value = LEARNING_RATE
            self.learning_shader['outcome'].value = outcome
            
            # Run learning shader (updates knowledge)
            self.learning_shader.run(
                group_x=TEXTURE_SIZE[0] // 16,
                group_y=TEXTURE_SIZE[1] // 16
            )
            
            # Swap: updated becomes current
            temp = self.knowledge_texture
            self.knowledge_texture = updated_knowledge_texture
            updated_knowledge_texture = temp
            
            # Read updated knowledge back
            updated_data = np.frombuffer(self.knowledge_texture.read(), dtype=np.float32)
            updated_knowledge_texture.write(updated_data.tobytes())
        
        # Final: save updated knowledge to BASE_BRAIN.png
        final_knowledge = np.frombuffer(self.knowledge_texture.read(), dtype=np.float32)
        final_knowledge = final_knowledge.reshape((256, 256, 4))
        
        # Save to disk as PNG
        self.brain_manager.save_brain_state(final_knowledge, 'base')
        
        # Cleanup
        updated_knowledge_texture.release()
        
        print(f"[LEARNING] Brain updated after game (outcome: {outcome:+.1f}, "
              f"{len(self.game_positions)} positions learned)")
        ...
        while running:
            ...
            if not board.white_to_move and not chimera_thinking:
                ...
                if game_over:
                    ...
                    # Find best move (increased depth for better play)
                    best_move = engine.find_best_move(board.board, board.white_to_move, depth=4, game_move_count=board.move_count)
